*&---------------------------------------------------------------------*
*& Report  ZOTCP0120O_ORDER_ACK_FORM
*&---------------------------------------------------------------------*
************************************************************************
* Program    : ZOTCP0120O_ORDER_ACK_FORM                               *
...
REPORT  zotcp0012o_order_ack_form.
...
*&--Merging Item data and Batches data
    LOOP AT li_vbap ASSIGNING <lfs_vbap>.
      ...
      lwa_item-netwr  = <lfs_vbap>-netwr.
      lwa_item-waerk  = <lfs_vbap>-waerk.

      CLEAR: lv_object.
      REFRESH: li_lines[].
      CONCATENATE fp_vbeln <lfs_vbap>-posnr INTO lv_object.
...
*&--Fetch Terms of Payment Key at Header level
  SELECT SINGLE inco2    "Incoterms (Part 2)
